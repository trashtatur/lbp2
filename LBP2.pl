...
collect_time(Liste) :- findall((Vorname,Name,Gesamtspielzeit),
								(komponist(KNr, Vorname, Name, _Geboren, _Gestorben), KNr\=null,
								 ...
								 %Templiste enth�lt jetzt die Gesamtspielzeiten einzeln und listet so jeden Komponisten mehrmals.
								 %Die Einzelergebnisse müssen jetzt zusammengefasst und als neue Liste ausgegeben werden
